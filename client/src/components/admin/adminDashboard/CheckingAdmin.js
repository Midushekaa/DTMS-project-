...
export default function CheckingAdmin() {
  ...
  const handleAction = async (record, actionType) => {
    ...
    try {
      ...
      const url = `${process.env.REACT_APP_API_URL}/admin/check-application/${record._id}`;
  ...
  return (
    <div style={{ padding: "24px" }} className="text-left">
      ...
      <br />
      <Card title="Pending Transfer Applications" bordered>
        <Table
          ...
          loading={loading}
          pagination={{ pageSize: 10 }}
          locale={{ emptyText: "No pending applications available." }}
        />
      </Card>
    </div>
  );
